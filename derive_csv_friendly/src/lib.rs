...
/// Vec of other types are not supported and left as is.
...
pub fn csv_friendly(token_input: TokenStream) -> TokenStream {
    ...
    let csv_friendly_fields = orig_fields.iter().map(|f| {
        ...
        let new_ty = if is_string_vec(ty) {
            quote! { String }
        } else {
            quote! { #ty }
        };
    ...
    let from_orig_fields = orig_fields.iter().map(|field| {
        ...
        if is_string_vec(&field.ty) {
            quote! {
                #name: validate_and_join(orig.#name)
            }
        } else {
            quote! {
                #name: orig.#name
    ...
    let from_csv_friendly_fields = orig_fields.iter().map(|field| {
        ...
        if is_string_vec(&field.ty) {
            quote! {
                #name: csv_friendly.#name.split("|").map(|s| s.into()).collect()
            }
        } else {
            quote! {
                #name: csv_friendly.#name
            }
        }
    });
